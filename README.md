...
Graded moves show a circular SVG badge at the destination, with matching source/destination highlights. Badges follow orientation and main-line/variation navigation. Labels: Brilliant, Great, Best, Excellent, Good, Inaccuracy, Mistake, Blunder, Book, Forced, Miss and Missed Win. Missing adjacent analysis is Pending, never a fabricated grade. Ordinary shallow completed searches receive provisional grades; Brilliant and Great still require depth 12.

### Local opening knowledge and grading

The self-hosted opening index contains 3,815 named openings/variations covering ECO A00?E99, from lichess-org/chess-openings commit `c67912be581f0793dbaa776be5ccf111e01f88d9` (CC0). Vendored TSV sources are in `data/openings`; `scripts/build-openings.mjs` replays them with chess.js and generates `public/openings/index.json`, COPYING and a hash manifest. Builds and runtime matching need no external opening service. The existing data worker loads and validates the index. Position matching includes turn/castling/en-passant state, recognizes transpositions, and retains the last named opening after leaving the line. This is a named-opening catalogue, not exhaustive chess theory; its longest line is 36 plies.

Classification priority: a missed legal mate in one is **Missed Win**; exactly one legal move is **Forced**; delivered checkmate is **Best**; a known opening continuation is **Book**. Book describes catalogue membership, not engine approval. These facts do not require an engine search.

**Miss** requires compatible exact centipawn results showing an opponent-created opportunity: the current mover previously had at most +0.50, then at least +2.00, and their move returns to +0.50 or worse with loss at least 1.50. This is our explicit heuristic, inspired by the general missed-opportunity definition, not a reproduction of platform formulas. Mate transitions remain separate. Ordinary grades use published expected-points-loss bands: Excellent ?0.02, Good ?0.05, Inaccuracy ?0.10, Mistake ?0.20, Blunder >0.20. Our rating-neutral approximation is `1 / (1 + exp(-moverCp / 400))`; it is not Chess.com's unpublished rating-dependent model. Prefer exact same-depth MultiPV scores from the same position; otherwise compare adjacent searches. The top engine move is always Best or a higher-priority special label, regardless of later search drift. Numerical before/after evaluation changes remain visible independently.

Brilliant allows best moves or same-position MultiPV-confirmed near-best moves (?30 cp and ?0.02 estimated loss), a healthy resulting position (at least -0.50 or a favorable mate), and the moved non-pawn piece being immediately legally capturable, then captured within six PV plies without moving again. At least two net material points must remain conceded even after an available immediate recapture. Ordinary trades, pawn moves, unrelated later sacrifices and adjacent-search drift cannot establish Brilliant. This deliberately conservative rule can miss indirect sacrifices. An established winning alternative (+6.00 or favorable mate) excludes it; without alternatives, the initial score is the conservative fallback. Evidence can come from the played move's MultiPV or the following position's principal line. Brilliant/Great require depth 12. Great takes precedence for critical top moves retaining a healthy position with at least 150 cp and 0.15 estimated expected-points separation from the next alternative. A mover-relative upper bound can prove that alternative is insufficient; opposite bounds cannot. Alternatives may be one completed depth shallower than the primary line, reflecting normal MultiPV reporting. These are transparent heuristics, not tactical proofs or guaranteed Chess.com-equivalent results.
