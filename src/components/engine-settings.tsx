...
export function EngineSettings() {
  ...
  return <div className="space-y-4 border-t pt-4">
    <h3 className="font-medium">Engine settings</h3>
    <div className="grid grid-cols-2 gap-3">
      ...
      <label className="text-sm">Principal variations<select aria-label="Principal variations count" className="mt-2 h-10 w-full rounded-md border bg-background px-2" value={preferences.multiPv} onChange={(event) => update({ multiPv: Number(event.target.value) })}>{[1, 2, 3, 4, 5].map((n) => <option key={n} value={n}>{n}</option>)}</select></label>
    </div>
