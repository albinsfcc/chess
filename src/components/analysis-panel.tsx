  ...
  return <section className="min-w-0 rounded-xl border bg-card p-5" aria-labelledby="analysis-heading">
    ...
    {analysis.result ? <div className="mt-4 space-y-3" aria-label="Current position analysis">
      ...
      <ol className="space-y-2" aria-label="Principal variations">
        {analysis.result.lines.map((line) => <li key={line.multiPv} className="rounded-md border bg-background/40 p-3 text-sm">
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent><DialogHeader><DialogTitle>Choose your analysis mode</DialogTitle><DialogDescription>Analyze only the current position. You control the moves and navigation.</DialogDescription></DialogHeader>
          <div className="space-y-3">
            <div className="rounded-lg border p-4"><h3 className="font-medium">Free play</h3><p className="my-2 text-sm text-muted-foreground">Play legal moves without automatic recommendations. Analyze Position remains available.</p><Button variant="outline" onClick={() => { analysis.start(false); if (useWorkspace.getState().imported) closeGame(); setOpen(false); }}>Use free play</Button></div>
