...
export const useAnalysis = create<AnalysisState>((set, get) => ({
  ...
  analyze: async (force = false) => {
    ...
    set({ enabled: true, error: null, result: null, cached: false });
    try {
      ...
      if (!force) {
        try {
          ...
          if (cached) { set({ result: cached, status: "ready", cached: true }); return; }
  ...
  restart: async () => {
    const token = ++serial; set({ enabled: false, result: null, error: null, status: "loading" });
  ...
  positionChanged: (newGame = false) => {
    ...
    set({ result: null, fen: null, cached: false, error: null, status: "stopped" });
...
export function connectAnalysis(): () => void {
  ...
  const offEngine = client.subscribe((event) => {
    ...
    else if (event.type === "error") useAnalysis.setState({ error: event.message, status: "error", result: null });
  });
  ...
  useAnalysis.getState().hydrate();
  return () => { offEngine(); offWorkspace(); useAnalysis.getState().stop(); client.terminate(); };
}
