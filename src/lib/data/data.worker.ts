...
scope.onmessage = async ({ data }) => {
  try {
    const value = data.kind === "validate" ? await validatePgn(data.payload.input, data.payload.existing) : data.kind === "backup" ? await validateBackup(data.payload) : await normalizePlatformGame(data.payload);
